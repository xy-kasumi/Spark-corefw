...
static int tmc_uart_read(size_t size) {
  if (size > TMC_UART_BUFFER_SIZE) {
    return -EINVAL;
  }
...
uint32_t tmc_tx_regread(uint8_t addr) {
  ...
  if (tmc_uart_write((uint8_t*)&request, sizeof(request)) < 0) {
    return 0;
  }
  ...
  k_event_clear(&tmc_uart_evt, TMC_UART_EVT_DONE);
  if (tmc_uart_read(sizeof(reply)) < 0) {
    return 0;
  }
  if (!k_event_wait(&tmc_uart_evt, TMC_UART_EVT_DONE, false, K_MSEC(15))) {
    return 0;
...
int tmc_tx_regwrite(uint8_t addr, uint32_t value) {
  ...
  request.crc = tmc_uart_crc((uint8_t*)&request, sizeof(request) - 1);
  int ret = tmc_uart_write((uint8_t*)&request, sizeof(request));
  if (ret < 0) {
    return ret;
  }
  if (!k_event_wait(&tmc_uart_evt, TMC_UART_EVT_DONE, false, K_MSEC(15))) {
