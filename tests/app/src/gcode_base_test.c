...
ZTEST(gcode_base, test_basic_g0_command) {
  ...
  bool result = parse_gcode(sl_from_str("G0"), &parsed);
...
ZTEST(gcode_base, test_g38_2_command) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("G38.2"), &parsed);
...
ZTEST(gcode_base, test_g38_3_command) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("G38.3"), &parsed);
...
ZTEST(gcode_base, test_g1_with_coordinates) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("G1 X10.5 Y-20.3 Z5"), &parsed);
...
ZTEST(gcode_base, test_g0_with_c_axis) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("G0 X10 Y20 C45.5"), &parsed);

  zassert_true(result, "G0 with C axis should parse successfully");
  zassert_equal(parsed.code, 0, "Code should be 0 for G0");
  zassert_equal(parsed.sub_code, -1, "Sub-code should be -1 for G0");
  zassert_equal(parsed.x_state, AXIS_WITH_VALUE, "X should have value");
  zassert_equal(parsed.y_state, AXIS_WITH_VALUE, "Y should have value");
  zassert_equal(parsed.c_state, AXIS_WITH_VALUE, "C should have value");
  zassert_equal(parsed.x, 10.0f, "X should be 10.0");
  zassert_equal(parsed.y, 20.0f, "Y should be 20.0");
  zassert_equal(parsed.c, 45.5f, "C should be 45.5");
}

ZTEST(gcode_base, test_g1_with_all_axes) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("G1 X1.5 Y2.5 Z3.5 C90"), &parsed);

  zassert_true(result, "G1 with all axes should parse successfully");
  zassert_equal(parsed.code, 1, "Code should be 1 for G1");
  zassert_equal(parsed.sub_code, -1, "Sub-code should be -1 for G1");
  zassert_equal(parsed.x_state, AXIS_WITH_VALUE, "X should have value");
  zassert_equal(parsed.y_state, AXIS_WITH_VALUE, "Y should have value");
  zassert_equal(parsed.z_state, AXIS_WITH_VALUE, "Z should have value");
  zassert_equal(parsed.c_state, AXIS_WITH_VALUE, "C should have value");
  zassert_equal(parsed.x, 1.5f, "X should be 1.5");
  zassert_equal(parsed.y, 2.5f, "Y should be 2.5");
  zassert_equal(parsed.z, 3.5f, "Z should be 3.5");
  zassert_equal(parsed.c, 90.0f, "C should be 90.0");
}

ZTEST(gcode_base, test_g28_axis_only) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("G28 X"), &parsed);
...
ZTEST(gcode_base, test_g28_c_axis) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("G28 C"), &parsed);

  zassert_true(result, "G28 C should parse successfully");
  zassert_equal(parsed.code, 28, "Code should be 28 for G28");
  zassert_equal(parsed.sub_code, -1, "Sub-code should be -1 for G28");
  zassert_equal(parsed.c_state, AXIS_ONLY, "C should be axis only");
  zassert_equal(parsed.x_state, AXIS_NOT_SPECIFIED,
                "X should not be specified");
  zassert_equal(parsed.y_state, AXIS_NOT_SPECIFIED,
                "Y should not be specified");
...
ZTEST(gcode_base, test_basic_m3_command) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("M3"), &parsed);
...
ZTEST(gcode_base, test_m3_with_all_parameters) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("M3 P750 Q1.5 R30"), &parsed);
...
ZTEST(gcode_base, test_m4_with_partial_parameters) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("M4 Q2.0 R25"), &parsed);
...
ZTEST(gcode_base, test_m5_command) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("M5"), &parsed);
...
ZTEST(gcode_base, test_m_code_mixed_parameters) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("M3 P1000 R50"), &parsed);
...
ZTEST(gcode_base, test_invalid_m_code_parameter) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("M3 P"), &parsed);
...
ZTEST(gcode_base, test_m_code_with_unknown_parameter) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("M3 P500 S100"), &parsed);
...
ZTEST(gcode_base, test_invalid_m_code_number) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("M999"), &parsed);
...
ZTEST(gcode_base, test_empty_string) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str(""), &parsed);
...
ZTEST(gcode_base, test_extra_whitespace_success) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("G0   X10.5    Y20"), &parsed);
...
ZTEST(gcode_base, test_lowercase_command) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("g0 X10"), &parsed);
...
ZTEST(gcode_base, test_garbled_command) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("G0abc X10"), &parsed);
...
ZTEST(gcode_base, test_garbled_number) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("G0 X10.5.2"), &parsed);
...
ZTEST(gcode_base, test_no_whitespace_between_params) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("G0X1Y2"), &parsed);
...
ZTEST(gcode_base, test_invalid_decimal_format) {
  gcode_parsed_t parsed;
  bool result = parse_gcode(sl_from_str("M3 P10..5"), &parsed);
